...
void readFromTXT() {
    ...
    for (auto cord_house : cord_houses) {
        map<string, float> ratio_dis;//массивы рассто€ний и коэф рассто€ний
        map<string, int> dis;
        int children;
        if (INPUT)
            children = cord_house.second;
        else
            children = rand() % COUNT_CHILDREN + 1;//случайна€ генераци€ кол-ва детей от 1 до 20
        sum_children += children;
        all_houses.push_back({ {cord_house.first},{children,id} });
        for (auto cord_school : cord_schools) {//расчет рассто€ни€ от дома до каждой школы
            ...
            ratio_dis[cord_school] = float(children) / dis[cord_school];//расчет коэф рассто€ни€ от дома до каждой школы
        }
...
//        auto tmp = house.first.getMin_dis();//минимальное рассто€ние
